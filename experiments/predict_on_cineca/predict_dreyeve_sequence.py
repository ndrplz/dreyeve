...
if __name__ == '__main__':
    ...
    for sample in xrange(15, 7500 - 1):
        ...
        Y_dreyevenet = dreyevenet_model.predict(X)[0][0]  # get only [fine_out][remove batch]
        Y_image = image_branch.predict(X[:3])[0][0]  # predict on image
        Y_flow = flow_branch.predict(X[3:6])[0][0]  # predict on optical flow
        Y_semseg = semseg_branch.predict(X[6:])[0][0]  # predict on segmentation
        ...
        if verbose:
            ...
            y_stitch = stitch_together([np.tile(normalize(Y_image.transpose(1, 2, 0)), reps=(1, 1, 3)),
                                        np.tile(normalize(Y_flow.transpose(1, 2, 0)), reps=(1, 1, 3)),
                                        np.tile(normalize(Y_semseg.transpose(1, 2, 0)), reps=(1, 1, 3))],
                                       layout=(3, 1), resize_dim=(720, 720))

            y_tot = np.tile(normalize(resize_tensor(Y_dreyevenet, new_size=(720, 720)).transpose(1, 2, 0)),
                            reps=(1, 1, 3))
